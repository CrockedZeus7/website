window.addEventListener("load", () => {
    ...
    const year = 2023; // 연도를 2023으로 고정
    const lunchDataUrl = 'https://open.neis.go.kr/hub/mealServiceDietInfo?KEY=3b032261176042658cad62c6d0e8355c&Type=json&plndex=1&pSize=30&ATPT_OFCDC_SC_CODE=B10&SD_SCHUL_CODE=7041168';
    ...
    selectedDate.addEventListener('change', () => {
      ...
      fetch(lunchDataByDateUrl) //API 받아오기
        ...
        .then(data => {
          ...
          const textWithMilk = text.replace(/12./g, '').replace(/2./g, '(🥛)').replace(/[\d.()]+/g, '').replace(/o|l|m|g/g, '').replace(/[.()]/g, '').replace(/🥛/g, '(🥛)').trim();
